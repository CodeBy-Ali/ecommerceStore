...
import { calculateProductSubtotal as calculateCartSubTotal, getShippingConfig } from "../utils/cartUtils.ts";
...
async function removeOrderedItemsFromCart(cartId:mongoose.Types.ObjectId|string) {
  try {
...
async function getOrderInfo(
  ...
  if (!shippingAddressId || !mongoose.isValidObjectId(shippingAddressId)) {
    throw createError(404, "User Order shipping address not found", "fail");
  }  

  const userCart = await Cart.findOne({ _id: reqBody?.cartId });
  if (!userCart) throw createError(404, "User Cart not found", "fail");
